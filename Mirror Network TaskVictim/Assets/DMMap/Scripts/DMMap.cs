...
namespace DMM {
    public class DMMap : MonoBehaviour {
        ...
        private void SetupCamera() {
            ...
            DMMapCamera.transform.position = camPos;
            ...
            q.eulerAngles = rot;
            DMMapCamera.transform.rotation = q;
        ...
        private void InitializeRenderTexture() {
            ...
            mainRenderTexture = new RenderTexture(1024, 1024, 24, RenderTextureFormat.ARGB32);
        ...
        private void GenerateMeshLayer(int layer = 0) {
            ...
            //Triangulation
            List<TriangleNet.Mesh> tMesh = new List<TriangleNet.Mesh>();
            //List<TriangleNet.Geometry.Polygon> tp = new List<Polygon>();
            for (int i = 0; i < dPolygons.Count; i++) {
                if (!dPolygons[i].isHole) {
                    TriangleNet.Geometry.Polygon tnPoly = new Polygon();
                    ...
                    TriangleNet.Meshing.GenericMesher gm = new TriangleNet.Meshing.GenericMesher();
                    gm.Triangulate(tnPoly);
                    tMesh.Add((TriangleNet.Mesh)tnPoly.Triangulate());
                }
            }
